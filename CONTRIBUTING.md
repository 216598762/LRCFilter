# Contributing to LRCFilter

Thank you for your interest in contributing to LRCFilter! This document provides guidelines and information for contributors.
...
2. **Clone your fork** locally:
   ```bash
   git clone https://github.com/YOUR_USERNAME/lrcfilter.git
   cd lrcfilter
   ```
3. **Add upstream remote**:
   ```bash
   git remote add upstream https://github.com/216598762/lrcfilter.git
   ```
...
# Run with coverage
pytest tests/ --cov=lrcfilter --cov-report=term-missing

# Run specific test file
pytest tests/test_config_wiring.py -v

# Run specific test
pytest tests/test_config_wiring.py::TestPipelineConfigValidation::test_valid_config -v
```
...
```
feat(analyzer): add word-level timestamps support

fix(censorship): handle empty lyrics gracefully

docs(readme): update installation instructions

test(pipeline): add unit tests for config wiring
...
- `analyzer` - Whisper analyzer module
- `censorship` - Censorship detection
- `instrumental` - Instrumental detection
- `mismatch` - Metadata mismatch detection
...
1. Open a [discussion](https://github.com/216598762/lrcfilter/discussions)
2. Comment on an existing issue
3. Reach out to maintainers

Thank you for contributing to LRCFilter! 🎵
